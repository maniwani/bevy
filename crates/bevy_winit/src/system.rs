...
use crate::{
    ...
    get_best_videomode, get_fitting_videomode, CreateWindowParams, EventLoopWindowTarget,
...
pub fn create_windows<F: QueryFilter + 'static>(
    ...
    mut created_windows: Query<(Entity, &mut Window), F>,
    ...
    main_thread.run(|tls| {
        tls.resource_scope(|tls, mut winit_windows: Mut<WinitWindows>| {
            tls.resource_scope(|tls, mut adapters: Mut<AccessKitAdapters>| {
                ...
                let event_loop = unsafe { tls.resource::<EventLoopWindowTarget<AppEvent>>().get() };
    ...
    main_thread.run(|tls| {
        ...
        for window in closed_windows.read() {
            ...
            if !window_entities.contains(window) {
                ...
                window_closed_events.send(WindowClosed { window });
            }
        }
    });
}

...
///
/// In the [`RenderSet::Extract`](crate::RenderSet::Extract) step the asset is transferred
...
#[derive(Clone, Copy, Hash, Debug, PartialEq, Eq, SystemLabel)]
...
///
/// Therefore it sets up the [`RenderSet::Extract`](crate::RenderSet::Extract) and
...
impl<A: RenderAsset> Plugin for RenderAssetPlugin<A> {
    fn build(&self, app: &mut App) {
        if let Ok(render_app) = app.get_sub_app_mut(RenderApp) {
            let prepare_asset_system = prepare_assets::<A>.to(self.prepare_asset_label);
            ...
            render_app
                ...
                .add_system(extract_render_asset::<A>.to(RenderSet::Extract))
                .add_system(prepare_asset_system.to(RenderSet::Prepare));
        }
    }
}

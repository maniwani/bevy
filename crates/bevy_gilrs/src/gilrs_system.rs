...
use bevy_ecs::prelude::{Mut, Res, ResMut, ThreadLocal};
...
pub fn gilrs_event_startup_system(
    ...
    main_thread.run(|tls| {
        tls.resource_scope(|_tls, gilrs: Mut<crate::Gilrs>| {
...
pub fn gilrs_event_system(
    ...
    main_thread.run(|tls| {
        tls.resource_scope(|_tls, mut gilrs: Mut<crate::Gilrs>| {
                ...
                .filter_ev(&axis_dpad_to_button, &mut gilrs)
            {
                ...
                match gilrs_event.event {
                    ...
                    EventType::Disconnected => events.send(
                    ...
                    EventType::ButtonChanged(gilrs_button, raw_value, _) => {
                        if let Some(button_type) = convert_button(gilrs_button) {
                            ...
                            if let Some(filtered_value) =
                                button_settings.filter(raw_value, old_value)
                            {
                                events.send(
                                ...
                                // Update the current value prematurely so that `old_value` is
                                // correct in future iterations of the loop.
